...
namespace BattleSystem
{
    public class CameraFollow : MonoBehaviour
    {
        ...
        public Vector3 minValCave, maxValCave;
        ...
        void Follow1()
        {
            Vector3 targetPos = target.position + offset;
            Vector3 boundPos = new Vector3(
                Mathf.Clamp(targetPos.x, minValCave.x, maxValCave.x),
                Mathf.Clamp(targetPos.y, minValCave.y, maxValCave.y),
                Mathf.Clamp(targetPos.z, minValCave.z, maxValCave.z)
                );
            Vector3 smoothPos = Vector3.Lerp(transform.position, boundPos, smooth * Time.fixedDeltaTime);
            transform.position = smoothPos;
        }
